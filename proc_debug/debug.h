#define INITIAL_MEMORY {};

void dump_core(uint8_t v[], uint16_t i, uint8_t snd, uint8_t tmr, uint16_t pc, uint16_t instr, uint8_t sp, uint16_t stack[], uint16_t nnn, uint8_t x, uint8_t y, uint8_t kk, uint8_t n){
    printf("PC: %#06x\nI: %#06x\nSND: %#06x\nTMR: %#06x\nSP: %#06x\nINSTR: %#06x\nNNN: %#06x\nX: %#06x\nY: %#06x\nKK: %#06x\nN: %#06x\n", pc, i, snd, tmr, sp, instr, nnn, x, y, kk, n);
    printf("\nV:");
    for (int j = 0; j < 16; j++){
        printf("\n    %02d: %#06x", j, v[j]);
    }
    printf("\nStack:");
    for (int j = 0; j < 16; j++){
        printf("\n    %02d: %#06x", j, stack[j]);
    }
    printf("\n");
}

void print_scr(uint64_t vram[]){
    printf("\n");
    printf("\n");
    printf("\n");
    for (int i = 0; i < 32; i++){
        for (int j = 0; j < 64; j++){
            if (vram[i] & (((uint64_t)1) << (63-j))){
                printf("%%");
            } else {
                printf(".");
            }
        }
        printf("\n");
    }
    printf("\n");
    printf("\n");
    printf("\n");
}

#define FONT_DATA {\
0xF0, 0x90, 0x90, 0x90, 0xF0,\
0x20, 0x60, 0x20, 0x20, 0x70,\
0xF0, 0x10, 0xF0, 0x80, 0xF0,\
0xF0, 0x10, 0xF0, 0x10, 0xF0,\
0x90, 0x90, 0xF0, 0x10, 0x10,\
0xF0, 0x80, 0xF0, 0x10, 0xF0,\
0xF0, 0x80, 0xF0, 0x90, 0xF0,\
0xF0, 0x10, 0x20, 0x40, 0x40,\
0xF0, 0x90, 0xF0, 0x90, 0xF0,\
0xF0, 0x90, 0xF0, 0x10, 0xF0,\
0xF0, 0x90, 0xF0, 0x90, 0x90,\
0xE0, 0x90, 0xE0, 0x90, 0xE0,\
0xF0, 0x80, 0x80, 0x80, 0xF0,\
0xE0, 0x90, 0x90, 0x90, 0xE0,\
0xF0, 0x80, 0xF0, 0x80, 0xF0,\
0xF0, 0x80, 0xF0, 0x80, 0x80}