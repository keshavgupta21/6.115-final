...
void clk_timer_Start(void) 
{
    ...
    clk_timer_CLKEN |= clk_timer_CLKEN_MASK;
	clk_timer_CLKSTBY |= clk_timer_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clk_timer_Stop
********************************************************************************
*
* Summary:
...
void clk_timer_Stop(void) 
{
    ...
    clk_timer_CLKEN &= (uint8)(~clk_timer_CLKEN_MASK);
	clk_timer_CLKSTBY &= (uint8)(~clk_timer_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clk_timer_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clk_timer_CLKEN & clk_timer_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clk_timer_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clk_timer_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* clk_timer__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(clk_timer_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clk_timer_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clk_timer_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clk_timer_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clk_timer_GetSourceRegister();
    uint16 oldDivider = clk_timer_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clk_timer_CLKEN & clk_timer_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clk_timer_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clk_timer_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clk_timer_DIV_PTR);
}


/*******************************************************************************
* Function Name: clk_timer_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clk_timer_MOD_SRC |= modeBitMask & (uint8)clk_timer_MODE_MASK;
}


/*******************************************************************************
* Function Name: clk_timer_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clk_timer_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clk_timer_MODE_MASK));
}


/*******************************************************************************
* Function Name: clk_timer_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clk_timer_MOD_SRC & (uint8)(clk_timer_MODE_MASK);
}


/*******************************************************************************
* Function Name: clk_timer_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clk_timer_GetDividerRegister();
    uint8 oldSrc = clk_timer_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clk_timer_MOD_SRC |= CYCLK_SSS;
        clk_timer_MOD_SRC =
            (clk_timer_MOD_SRC & (uint8)(~clk_timer_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clk_timer_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clk_timer_MOD_SRC & clk_timer_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clk_timer_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clk_timer_PHASE = clkPhase & clk_timer_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clk_timer_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clk_timer_PHASE & clk_timer_PHASE_MASK;
}
