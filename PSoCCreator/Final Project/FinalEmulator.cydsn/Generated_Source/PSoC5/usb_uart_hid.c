...
volatile uint8 usb_uart_hidProtocol[usb_uart_MAX_INTERFACES_NUMBER];
 ...
 * reload timer.*/
volatile uint8 usb_uart_hidIdleRate[usb_uart_MAX_INTERFACES_NUMBER];
...
uint8 usb_uart_DispatchHIDClassRqst(void) 
{
    ...
    uint8 interfaceNumber = (uint8) usb_uart_wIndexLoReg;
    ...
    if (0u != (usb_uart_bmRequestTypeReg & usb_uart_RQST_DIR_D2H))
    {
        ...
        switch (usb_uart_bRequestReg)
        {
            case usb_uart_GET_DESCRIPTOR:
                if (usb_uart_wValueHiReg == usb_uart_DESCR_HID_CLASS)
                {
                ...
                else if (usb_uart_wValueHiReg == usb_uart_DESCR_HID_REPORT)
                {
            ...
            case usb_uart_HID_GET_IDLE:
                ...
                if ((interfaceNumber < usb_uart_MAX_INTERFACES_NUMBER) && (usb_uart_wValueLoReg == 0u)) 
                {
    ...
    {   
        ...
        switch (usb_uart_bRequestReg)
        {
            ...
            case usb_uart_HID_SET_IDLE:
                ...
                if ((interfaceNumber < usb_uart_MAX_INTERFACES_NUMBER) && (usb_uart_wValueLoReg == 0u))
                {
                    usb_uart_hidIdleRate[interfaceNumber] = (uint8)usb_uart_wValueHiReg;
            ...
            case usb_uart_HID_SET_PROTOCOL:
                ...
                if ((interfaceNumber < usb_uart_MAX_INTERFACES_NUMBER) && (usb_uart_wValueLoReg <= 1u))
                {
                    usb_uart_hidProtocol[interfaceNumber] = (uint8)usb_uart_wValueLoReg;
                    requestHandled = usb_uart_InitNoDataControlTransfer();
                }
                break;
...
void usb_uart_FindHidClassDecriptor(void) 
{
    ...
    interfaceN = (uint8) usb_uart_wIndexLoReg;
...
void usb_uart_FindReportDescriptor(void) 
{
    ...
    usb_uart_currentTD.count =  ((uint16)((uint16) pDescr[1u] << 8u) | pDescr[0u]);
...
void usb_uart_FindReport(void) 
{
...
#ifdef usb_uart_FIND_REPORT_CALLBACK
    usb_uart_FindReport_Callback();
#endif /* (usb_uart_FIND_REPORT_CALLBACK) */
    
    usb_uart_currentTD.count = 0u;   /* Init not supported condition */
    pTmp = usb_uart_GetConfigTablePtr(usb_uart_configuration - 1u);
    reportType = (uint8) usb_uart_wValueHiReg;
    interfaceN = (uint8) usb_uart_wIndexLoReg;
    ...
    if (interfaceN < usb_uart_MAX_INTERFACES_NUMBER)
    {
            ...
            (reportType <= usb_uart_HID_GET_REPORT_FEATURE))
        {
            ...
            /* Get reportID */
            reportType = (uint8) usb_uart_wValueLoReg;
