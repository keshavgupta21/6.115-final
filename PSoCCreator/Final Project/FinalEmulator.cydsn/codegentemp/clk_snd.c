...
void clk_snd_Start(void) 
{
    ...
    clk_snd_CLKEN |= clk_snd_CLKEN_MASK;
	clk_snd_CLKSTBY |= clk_snd_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clk_snd_Stop
********************************************************************************
*
* Summary:
...
void clk_snd_Stop(void) 
{
    ...
    clk_snd_CLKEN &= (uint8)(~clk_snd_CLKEN_MASK);
	clk_snd_CLKSTBY &= (uint8)(~clk_snd_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clk_snd_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clk_snd_CLKEN & clk_snd_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clk_snd_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clk_snd_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(clk_snd_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clk_snd_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clk_snd_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clk_snd_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clk_snd_GetSourceRegister();
    uint16 oldDivider = clk_snd_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clk_snd_CLKEN & clk_snd_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clk_snd_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clk_snd_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clk_snd_DIV_PTR);
}


/*******************************************************************************
* Function Name: clk_snd_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clk_snd_MOD_SRC |= modeBitMask & (uint8)clk_snd_MODE_MASK;
}


/*******************************************************************************
* Function Name: clk_snd_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clk_snd_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clk_snd_MODE_MASK));
}


/*******************************************************************************
* Function Name: clk_snd_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clk_snd_MOD_SRC & (uint8)(clk_snd_MODE_MASK);
}


/*******************************************************************************
* Function Name: clk_snd_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clk_snd_GetDividerRegister();
    uint8 oldSrc = clk_snd_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clk_snd_MOD_SRC |= CYCLK_SSS;
        clk_snd_MOD_SRC =
            (clk_snd_MOD_SRC & (uint8)(~clk_snd_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clk_snd_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clk_snd_MOD_SRC & clk_snd_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clk_snd_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clk_snd_PHASE = clkPhase & clk_snd_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clk_snd_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clk_snd_PHASE & clk_snd_PHASE_MASK;
}
